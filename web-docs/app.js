(() => {
  ...
  function renderNavigation() {
    const coreNavigation = catalog.phases.map(phase => {
      const docs = catalog.core
        .filter(doc => doc.phase === phase.id)
        .sort((left, right) => left.order - right.order);
      const topLevelDocs = docs.filter(doc => !doc.parentId);
      return renderNavigationSection(phase.label, topLevelDocs, docs);
    }).join('');
    const deepNavigation = catalog.topics.map(topic => {
      const docs = catalog.deep
        .filter(doc => doc.topic === topic.id)
        .sort((left, right) => left.order - right.order);
      return docs.length ? renderNavigationSection(topic.label, docs, docs, 'is-deep-section') : '';
    }).join('');
    elements.nav.innerHTML = `${coreNavigation}<div class="nav-divider" aria-hidden="true"></div>${deepNavigation}`;
    updateActiveNavigation();
    refreshIcons(elements.nav);
  }

  function renderNavigationSection(label, topLevelDocs, docs, extraClass = '') {
    return `
      <section class="nav-phase${extraClass ? ` ${extraClass}` : ''}">
        <span class="nav-phase-label">${escapeHtml(label)}</span>
