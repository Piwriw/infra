...
> 截止日期：2026-07-09（分支 `learn/brain`）
...
| sqlc 查询 | `packages/db/queries/user_identities/upsert_public_identity.sql` 等 | 新增 `GetUserIdentity`、`UpsertPublicIdentity` |
...
| OpenAPI | `spec/openapi.yml`、`spec/openapi-dashboard.yml`、`packages/*/internal/api/api.gen.go` | 新增 `AuthProviderTokenAuth`（Bearer）与 `AuthProviderTeamAuth`（`X-Team-ID`），与既有 Supabase scheme 并存。Header 大小写经 [#2723](https://github.com/e2b-dev/infra/pull/2723) 统一为 `X-Team-ID`（`packages/auth/pkg/auth/consts.go:HeaderTeamID`） |
...
| 文件 | 影响 |
|---|---|
| `packages/dashboard-api/internal/userprofile/ory.go` | **新建** Ory Kratos 项目 API 客户端（275 行）：通过 `ORY_PROJECT_API_TOKEN` 调用 Ory Admin API 拉取 identities |
...
| `packages/db/queries/user_identities/get_user_identities_by_subjects.sql`、`get_user_identities_by_user_ids.sql` | 新增批量查询（按 subjects / 按 user_ids） |
...
| `packages/db/queries/user_identities/upsert_public_identity.sql` + 生成代码 | 调整 upsert 字段顺序，配合 bootstrap 流程 |
...
| `packages/dashboard-api/internal/userprofile/supabase.go`、`supabase_test.go`、`mode.go`、`creator_context.go` | 删除 Supabase provider 与「双模式」开关；profile 解析变为 Ory-only |
...
| `AuthProviderTokenAuth` | `Authorization: Bearer …`（OIDC JWT） | [#2673](https://github.com/e2b-dev/infra/pull/2673) 引入 |
...
| 时间 | 路由 | 说明 |
|---|---|---|
| 2026-05-19 (`b673a10cb`, #2743) | `GET/POST /admin/auth-provider-profiles/*` | 管理 OIDC profile 元数据（白名单 issuer、display name 等） |
| 2026-06-01 (`6a7a59ee0`) | `POST /admin/users/bootstrap` | OIDC 用户预置 |
| 2026-06-15 (`ecc1291ad`) | `DELETE /admin/users/{userId}` | 删除用户（Ory 模式） |

### 4.6 IaC 层

- `iac/provider-gcp/main.tf`：`locals.auth_provider_config` 默认 `{ jwt = [] }`，从 `var.auth_provider_config` 注入。
...
- sqlc 查询：`packages/db/queries/user_identities/upsert_public_identity.sql`、`get_user_identities_by_subjects.sql`、`get_user_identities_by_user_ids.sql`
...
- 已删除（仅供历史参考）：`packages/auth/pkg/auth/legacy/`、`packages/auth/pkg/auth/jwt.go`、`packages/db/pkg/supabase/`、`packages/dashboard-api/internal/userprofile/supabase.go`、`packages/dashboard-api/internal/userprofile/mode.go`
