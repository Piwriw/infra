...
| 旧（2026.16） | 新（2026.28） | 用途 |
| --- | --- | --- |
| `Supabase1TokenAuth` | `AuthProviderBearerAuth` | 普通用户 Bearer Token（HTTP Bearer） |
| `Supabase2TeamAuth` | `AuthProviderTeamAuth` | 团队上下文 Header（`X-Team-ID`） |
| `AdminTokenAuth` | `AdminApiKeyAuth` | 管理面 API Key（`X-Admin-Token`） |
| —（新增） | `AdminTeamAuth` | 管理面团队上下文（`X-Team-ID`） |

- 涉及所有 endpoint 的 `security:` 段；HTTP Header **大小写修正**：`X-Team-Id` → `X-Team-ID`（#2723）。
- 命名约定：Bearer/ApiKey 字母序在前，确保 token 校验先于 team 上下文填充。
- 相关提交：#2673（OIDC Auth Provider）、#2720（整合 NewAuthService）、#2725/#2726/#2728（auth 包内聚）、#2934（admin token team auth）、#3042（删除 Supabase 残留）。
...
1. **认证 header 大小写**：`X-Team-Id` → `X-Team-ID`。
2. **Supabase token 移除**：改用 API Key 或 OIDC Auth Provider Bearer。
3. **Access Token 进入弃用**：尽快迁移到 `E2B_API_KEY`，否则将来会收到 `410 Gone`。
4. **Admin 路由鉴权变更**：从 `X-Admin-Token` 切到 `AdminApiKeyAuth` 体系（同一 header，但语义归属变化，且需配套 `X-Team-ID`）。
5. **User email 字段**：准备 email 变 `null` 的兼容代码。

**可选但推荐**：

6. 网络 API：开始使用 `egressProxy`（SOCKS5 BYOP）、`rules`（per-domain header 注入）、`allow_internet_access` 便捷字段。
7. 模板列表：切换到 `GET /v2/templates` 以获得分页与游标。
8. 沙箱 pause：评估 filesystem-only snapshot（`memory=false` / `autoPauseMemory=false`）能否降低成本，注意不能与 autoResume 共存。
9. 节点面：消费 `statusChangedAt` 与 hugepage 指标。
